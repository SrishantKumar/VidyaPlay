...
export const Road: React.FC<RoadProps> = ({ width, height, scrollOffset }) => {
  ...
  for (let x = -period + offset; x < width + period; x += period) {
    dashes.push(
      <Rect key={`d1-${x}`} x={x} y={divider1Y - LANE_MARK_H / 2} width={LANE_MARK_W} height={LANE_MARK_H} color={COLORS.LANE_LINE} opacity={0.4} />,
      <Rect key={`d2-${x}`} x={x} y={divider2Y - LANE_MARK_H / 2} width={LANE_MARK_W} height={LANE_MARK_H} color={COLORS.LANE_LINE} opacity={0.4} />
    );
  }
...
export const Car: React.FC<CarProps> = ({ x, laneCenterY, color, isBoosting, isTurbo }) => {
  ...
  return (
    <Group transform={[{ translateX: x }, { translateY: cy }]}>
      {(isBoosting || isTurbo) && (
        <Circle cx={w * 0.3} cy={h * 0.5} r={w * 0.6} color={isTurbo ? COLORS.TURBO_GLOW : COLORS.BOOST_GLOW} />
      )}
      ...
      <Rect x={0} y={h * 0.1} width={2} height={h * 0.3} color={color} />
...
export const SpeedLines: React.FC<{ canvasWidth: number; canvasHeight: number; opacity: number }> = ({ canvasWidth, canvasHeight, opacity }) => (
